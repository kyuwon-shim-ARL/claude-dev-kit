...
```python
# 분석 단계 자동 실행 로직
def execute_analysis(arguments):
    """5단계 완전 분석 사이클 실행"""
    
    # 1. 탐색: 현황 파악 및 데이터 수집
    context_data = explore_current_situation(arguments)
    
    # 2. 수렴: 패턴 식별 및 핵심 이슈 도출
    key_insights = analyze_patterns(context_data)
    
    # 3. 정제: 분석 결과 구조화 및 검증
    structured_analysis = refine_analysis(key_insights)
    
    # 4. 보고서: 자동 저장 및 문서화
    report_path = save_analysis_report(structured_analysis, arguments)
    
    # 5. 정리: 컨텍스트 아카이빙 및 다음 단계 추천
    recommendations = finalize_analysis(structured_analysis)
