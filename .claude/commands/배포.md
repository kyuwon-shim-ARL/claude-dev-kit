...
```python
# 배포 단계 자동 실행 로직
def execute_deployment(arguments):
    """플래그 기반 선택적 배포 실행"""
    ...
    # Dry-run 모드 체크
    if flags.get('dry_run') or flags.get('dry-run'):
        result['simulation'] = True
        result['would_clean'] = simulate_cleanup()
    ...
    if flags.get('cleanup') or flags.get('정리'):
        ...
        result['files_cleaned'] = cleanup_temporary_files()
        ...
        result['files_cleaned'] = cleanup_repository()
        result['docs_organized'] = False
        ...
        result['files_archived'] = organize_documentation()
        result['repo_cleaned'] = False
        result['stabilized'] = False
        result['git_committed'] = False
        result['git_pushed'] = False
        print("✅ 문서 정리 완료!")
        
    elif flags.get('stabilize'):
        # 안정화만
...
```bash
# 정리만 실행 (커밋/푸시 없음)
/배포 --cleanup         # 전체 정리 (레포+문서+안정화)
/배포 --정리            # 한글 별칭
/배포 -c               # 단축 별칭

# 부분 정리 옵션
/배포 --repo           # 레포정리만 (임시파일, 빌드아티팩트)
/배포 --docs           # 문서정리만 (아카이빙, 메타데이터)
/배포 --stabilize      # 안정화만 (의존성, import 최적화)

# 검증 옵션
/배포 --dry-run        # 시뮬레이션 (실제 변경 없음)
/배포 --no-commit      # 정리는 하되 커밋/푸시 안함
