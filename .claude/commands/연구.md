# /연구 - 통합 연구 관리 시스템 (Alias 지원)

## 사용법
```
/연구 [서브커맨드|별칭] [인자들]
```

## 🎯 서브커맨드 별칭 매핑

| 정식 명령어 | 별칭 (Alias) | 한글 | 설명 |
|------------|-------------|------|------|
| **init** | i, new | 시작, 초기화, 생성 | 새 프로젝트 시작 |
| **track** | t, log | 진행, 기록, 추적 | 진행사항 기록 |
| **experiment** | e, exp | 실험 | 실험 시작 |
| **hypothesis** | h, hyp | 가설 | 가설 설정 |
| **milestone** | m, mile | 마일스톤, 이정표 | 마일스톤 달성 |
| **checkpoint** | c, cp, save | 체크포인트, 저장 | 현재 상태 저장 |
| **status** | s, stat | 상태, 현황 | 프로젝트 상태 확인 |
| **list** | l, ls | 목록 | 프로젝트 목록 |
| **search** | f, find | 검색, 찾기 | 키워드 검색 |
| **backup** | b, bak | 백업 | 전체 백업 생성 |
| **report** | r, rep | 보고서 | 연구 보고서 생성 |
| **clean** | cl | 정리 | 임시 파일 정리 |
| **archive** | ar | 아카이브 | 완료 연구 보관 |
| **explore** | ex, exp, 탐구 | 탐구, 탐색 | 투명한 수렴 과정 탐구 |

## Claude 실행 프로토콜

```python
def execute_research(arguments):
    """연구 프로젝트 관리 시스템 실행"""
    # 실제 구현은 아래 Bash 스크립트를 통해 수행
    import subprocess
    import sys
    
    # Bash 스크립트 실행
    cmd = ['bash', '-c', f'''
# Bash 실행 로직 (아래 참조)
{" ".join(arguments)}
    ''']
    
    result = subprocess.run(cmd, capture_output=True, text=True)
    return {"success": result.returncode == 0, "output": result.stdout}

if __name__ == "__main__":
    import sys
    execute_research(sys.argv[1:])
```

## 실제 실행 프로토콜

```bash
#!/bin/bash

# 서브커맨드 정규화 함수
normalize_subcommand() {
    case "$1" in
        # init 별칭들
        i|new|시작|초기화|생성)
            echo "init"
            ;;
        
        # track 별칭들
        t|log|진행|기록|추적)
            echo "track"
            ;;
        
        # experiment 별칭들
        e|exp|실험)
            echo "experiment"
            ;;
        
        # hypothesis 별칭들
        h|hyp|가설)
            echo "hypothesis"
            ;;
        
        # milestone 별칭들
        m|mile|마일스톤|이정표)
            echo "milestone"
            ;;
        
        # checkpoint 별칭들
        c|cp|save|체크포인트|저장)
            echo "checkpoint"
            ;;
        
        # status 별칭들
        s|stat|상태|현황)
            echo "status"
            ;;
        
        # list 별칭들
        l|ls|목록)
            echo "list"
            ;;
        
        # search 별칭들
        f|find|검색|찾기)
            echo "search"
            ;;
        
        # backup 별칭들
        b|bak|백업)
            echo "backup"
            ;;
        
        # report 별칭들
        r|rep|보고서)
            echo "report"
            ;;
        
        # clean 별칭들
        cl|정리)
            echo "clean"
            ;;
        
        # archive 별칭들
        ar|아카이브)
            echo "archive"
            ;;
        
        # explore 별칭들
        ex|exp|탐구|탐색)
            echo "explore"
            ;;
        
        # 정식 명령어는 그대로 반환
        init|track|experiment|hypothesis|milestone|checkpoint|status|list|search|backup|report|clean|archive|explore)
            echo "$1"
            ;;
        
        # 인식 불가능한 명령
        *)
            echo "unknown"
            ;;
    esac
}

# 메인 실행 로직
SUBCOMMAND=$(normalize_subcommand "$1")
shift  # 첫 번째 인자 제거

if [ "$SUBCOMMAND" = "unknown" ]; then
    echo "❌ 알 수 없는 서브커맨드입니다."
    echo ""
    echo "사용 가능한 명령어:"
    echo "  init (i, new, 시작)        - 새 프로젝트"
    echo "  track (t, log, 진행)       - 진행 기록"
    echo "  experiment (e, exp, 실험)  - 실험 시작"
    echo "  hypothesis (h, hyp, 가설)  - 가설 설정"
    echo "  milestone (m, 마일스톤)     - 마일스톤"
    echo "  checkpoint (c, cp, 저장)   - 체크포인트"
    echo "  status (s, 상태)           - 현황 확인"
    echo "  list (l, ls, 목록)         - 프로젝트 목록"
    echo "  search (f, find, 검색)     - 검색"
    echo "  backup (b, bak, 백업)      - 백업"
    echo "  report (r, rep, 보고서)    - 연구 보고서"
    echo "  explore (ex, 탐구)         - 투명한 수렴 탐구"
    exit 1
fi

# 정규화된 명령어로 실행
case "$SUBCOMMAND" in
    init)
        PROJECT_NAME="$1"
        PROJECT_DESC="$2"
        PROJECT_DIR="research_projects/$(date +%Y-%m-%d)_${PROJECT_NAME}"
        
        mkdir -p "${PROJECT_DIR}"/{notebooks,data,results,figures,docs,scripts,experiments}
        
        cat > "${PROJECT_DIR}/README.md" << EOF
# ${PROJECT_NAME}
Created: $(date '+%Y-%m-%d %H:%M')
Description: ${PROJECT_DESC}
EOF
        
        cat > "${PROJECT_DIR}/timeline.md" << EOF
# Research Timeline - ${PROJECT_NAME}
## Project Start: $(date '+%Y-%m-%d %H:%M')
---
EOF
        
        echo "✅ 프로젝트 생성: ${PROJECT_DIR}"
        ;;
    
    track)
        ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
        if [ -z "$ACTIVE_PROJECT" ]; then
            echo "❌ 활성 프로젝트가 없습니다. 먼저 프로젝트를 생성하세요."
            exit 1
        fi
        
        MESSAGE="$*"
        cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
**Note**: ${MESSAGE}

---
EOF
        
        echo "✅ 진행사항 기록: ${MESSAGE}"
        ;;
    
    experiment)
        ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
        EXP_NAME="$1"
        EXP_DESC="$2"
        EXP_DIR="${ACTIVE_PROJECT}/experiments/$(date +%Y%m%d_%H%M)_${EXP_NAME}"
        
        mkdir -p "${EXP_DIR}"
        cat > "${EXP_DIR}/README.md" << EOF
# Experiment: ${EXP_NAME}
- Started: $(date '+%Y-%m-%d %H:%M')
- Description: ${EXP_DESC}
- Status: Running

## Setup
[실험 설정]

## Results
[결과 기록]
EOF
        
        # 타임라인에도 기록
        cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
🧪 **Experiment Started**: ${EXP_NAME}
${EXP_DESC}

---
EOF
        
        echo "✅ 실험 시작: ${EXP_DIR}"
        ;;
    
    hypothesis)
        ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
        HYPOTHESIS="$*"
        
        # 가설 파일 생성 (없으면)
        if [ ! -f "${ACTIVE_PROJECT}/hypotheses.md" ]; then
            echo "# Hypotheses" > "${ACTIVE_PROJECT}/hypotheses.md"
        fi
        
        cat >> "${ACTIVE_PROJECT}/hypotheses.md" << EOF

## $(date '+%Y-%m-%d')
**Hypothesis**: ${HYPOTHESIS}
**Status**: Active
**Validation**: Pending
---
EOF
        
        # 타임라인에도 기록
        cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
💡 **Hypothesis**: ${HYPOTHESIS}

---
EOF
        
        echo "✅ 가설 설정: ${HYPOTHESIS}"
        ;;
    
    milestone)
        ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
        MILESTONE_NAME="$1"
        shift
        MILESTONE_DESC="$*"
        
        # 마일스톤 파일 생성 (없으면)
        if [ ! -f "${ACTIVE_PROJECT}/milestones.md" ]; then
            echo "# Milestones" > "${ACTIVE_PROJECT}/milestones.md"
        fi
        
        cat >> "${ACTIVE_PROJECT}/milestones.md" << EOF

## 🎯 $(date '+%Y-%m-%d'): ${MILESTONE_NAME}
${MILESTONE_DESC}
---
EOF
        
        # 타임라인에도 기록
        cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
🎯 **Milestone Achieved**: ${MILESTONE_NAME}
${MILESTONE_DESC}

---
EOF
        
        echo "✅ 마일스톤 달성: ${MILESTONE_NAME}"
        ;;
    
    checkpoint)
        ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
        CHECKPOINT_ID="$(date +%Y%m%d_%H%M%S)"
        CHECKPOINT_DIR="${ACTIVE_PROJECT}/checkpoints/${CHECKPOINT_ID}"
        MEMO="$*"
        
        # 체크포인트 디렉토리 생성
        mkdir -p "${CHECKPOINT_DIR}"
        
        # 현재 상태 백업
        cp "${ACTIVE_PROJECT}/timeline.md" "${CHECKPOINT_DIR}/" 2>/dev/null
        [ -f "${ACTIVE_PROJECT}/hypotheses.md" ] && cp "${ACTIVE_PROJECT}/hypotheses.md" "${CHECKPOINT_DIR}/"
        [ -f "${ACTIVE_PROJECT}/milestones.md" ] && cp "${ACTIVE_PROJECT}/milestones.md" "${CHECKPOINT_DIR}/"
        
        # 체크포인트 정보 저장
        cat > "${CHECKPOINT_DIR}/checkpoint.info" << EOF
ID=${CHECKPOINT_ID}
DATE=$(date '+%Y-%m-%d %H:%M:%S')
MEMO=${MEMO}
EOF
        
        # 타임라인에도 기록
        cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
⏸️ **Checkpoint Created**: ${CHECKPOINT_ID}
${MEMO}

---
EOF
        
        echo "✅ 체크포인트 생성: ${CHECKPOINT_ID}"
        [ -n "$MEMO" ] && echo "   메모: ${MEMO}"
        ;;
    
    status)
        echo "📊 연구 프로젝트 현황"
        echo "===================="
        
        for PROJECT_DIR in research_projects/*/; do
            if [ -d "$PROJECT_DIR" ]; then
                PROJECT_NAME=$(basename "$PROJECT_DIR")
                
                # 통계 계산
                TIMELINE_ENTRIES=$(grep -c "^##" "${PROJECT_DIR}/timeline.md" 2>/dev/null || echo 0)
                EXP_COUNT=$(ls -d "${PROJECT_DIR}"/experiments/* 2>/dev/null | wc -l)
                CHECKPOINT_COUNT=$(ls -d "${PROJECT_DIR}"/checkpoints/* 2>/dev/null | wc -l)
                
                echo ""
                echo "📁 ${PROJECT_NAME}"
                echo "   진행: ${TIMELINE_ENTRIES} 항목"
                echo "   실험: ${EXP_COUNT}개"
                echo "   체크포인트: ${CHECKPOINT_COUNT}개"
                
                # 최근 활동
                LAST_ENTRY=$(grep "^##" "${PROJECT_DIR}/timeline.md" 2>/dev/null | tail -1)
                if [ -n "$LAST_ENTRY" ]; then
                    echo "   최근: ${LAST_ENTRY#\#\# }"
                fi
            fi
        done
        ;;
    
    list)
        echo "📋 전체 프로젝트 목록"
        echo "==================="
        
        for PROJECT_DIR in research_projects/*/; do
            if [ -d "$PROJECT_DIR" ]; then
                PROJECT_NAME=$(basename "$PROJECT_DIR")
                FILE_COUNT=$(find "$PROJECT_DIR" -type f | wc -l)
                SIZE=$(du -sh "$PROJECT_DIR" | cut -f1)
                
                # README에서 설명 추출
                if [ -f "${PROJECT_DIR}/README.md" ]; then
                    DESC=$(grep "Description:" "${PROJECT_DIR}/README.md" | cut -d: -f2- | xargs)
                else
                    DESC="(설명 없음)"
                fi
                
                echo ""
                echo "• ${PROJECT_NAME}"
                echo "  설명: ${DESC}"
                echo "  파일: ${FILE_COUNT}개, 크기: ${SIZE}"
            fi
        done
        ;;
    
    search)
        SEARCH_TERM="$*"
        echo "🔍 '${SEARCH_TERM}' 검색 결과"
        echo "========================"
        
        # 모든 프로젝트에서 검색
        grep -r "${SEARCH_TERM}" research_projects/ --include="*.md" 2>/dev/null | \
            while IFS=: read -r file content; do
                PROJECT=$(echo "$file" | cut -d'/' -f2)
                FILE_NAME=$(basename "$file")
                echo ""
                echo "📁 ${PROJECT} / ${FILE_NAME}"
                echo "   ${content:0:100}..."
            done
        ;;
    
    backup)
        BACKUP_NAME="research_backup_$(date +%Y%m%d_%H%M%S).tar.gz"
        tar -czf "${BACKUP_NAME}" research_projects/
        echo "✅ 백업 완료: ${BACKUP_NAME}"
        echo "   크기: $(du -h ${BACKUP_NAME} | cut -f1)"
        ;;
    
    report)
        PROJECT_NAME="$1"
        if [ -z "$PROJECT_NAME" ]; then
            # 활성 프로젝트 자동 감지
            ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
            if [ -z "$ACTIVE_PROJECT" ]; then
                echo "❌ 프로젝트를 지정하세요: /연구 report [프로젝트명]"
                echo "   또는 활성 프로젝트를 생성하세요."
                exit 1
            fi
            PROJECT_NAME=$(basename "$ACTIVE_PROJECT")
        fi
        
        echo "📊 ${PROJECT_NAME} 연구 보고서 생성 중..."
        
        # Python 스크립트가 있는지 확인
        REPORT_SCRIPT="src/research_report_generator.py"
        if [ ! -f "$REPORT_SCRIPT" ]; then
            echo "❌ 연구 보고서 생성기를 찾을 수 없습니다: $REPORT_SCRIPT"
            echo "   연구 보고서 시스템이 설치되지 않았습니다."
            exit 1
        fi
        
        # Python 실행 환경 확인
        if ! command -v python3 &> /dev/null; then
            echo "❌ Python3이 설치되지 않았습니다."
            exit 1
        fi
        
        # 보고서 생성 실행
        python3 "$REPORT_SCRIPT" "$PROJECT_NAME" "$(pwd)"
        
        if [ $? -eq 0 ]; then
            echo ""
            echo "🎉 연구 보고서 생성 완료!"
            echo "📁 위치: reports/research/${PROJECT_NAME}_latest_report.md"
            echo ""
            echo "💡 생성된 보고서:"
            echo "   - 📊 연구 개요 및 통계"
            echo "   - 🎯 가설 및 실험 요약"  
            echo "   - 📈 시간순 진행 과정"
            echo "   - 🏆 주요 성과 및 마일스톤"
            echo "   - 🔮 향후 계획"
        else
            echo "❌ 보고서 생성 중 오류가 발생했습니다."
        fi
        ;;
    
    clean)
        echo "🧹 연구 프로젝트 정리 시작..."
        
        # 임시 파일 정리
        TEMP_PATTERNS=("*.tmp" "*.cache" "*_temp.*" "*_test.*" ".ipynb_checkpoints")
        CLEANED_COUNT=0
        
        for pattern in "${TEMP_PATTERNS[@]}"; do
            while IFS= read -r file; do
                if [ -f "$file" ]; then
                    rm "$file"
                    ((CLEANED_COUNT++))
                    echo "  ✓ 삭제: $file"
                fi
            done < <(find . -name "$pattern" -type f 2>/dev/null)
        done
        
        # 빈 디렉토리 정리
        find . -type d -empty -delete 2>/dev/null
        
        echo "✅ 정리 완료: ${CLEANED_COUNT}개 파일 삭제"
        
        # 캐시 디렉토리 정리
        if [ -d "cache" ]; then
            echo "🗑️ 캐시 디렉토리 정리..."
            rm -rf cache/*
            echo "  ✓ 캐시 비움"
        fi
        ;;
    
    archive)
        PROJECT_NAME="$1"
        if [ -z "$PROJECT_NAME" ]; then
            echo "❌ 아카이브할 프로젝트를 지정하세요: /연구 archive [프로젝트명]"
            exit 1
        fi
        
        PROJECT_DIR="research_projects/${PROJECT_NAME}"
        if [ ! -d "$PROJECT_DIR" ]; then
            echo "❌ 프로젝트를 찾을 수 없습니다: $PROJECT_NAME"
            exit 1
        fi
        
        # 아카이브 디렉토리 생성
        ARCHIVE_DATE=$(date +%Y%m%d)
        ARCHIVE_DIR="archive/research/${PROJECT_NAME}_${ARCHIVE_DATE}"
        mkdir -p "$ARCHIVE_DIR"
        
        echo "📦 ${PROJECT_NAME} 프로젝트 아카이브 중..."
        
        # 프로젝트 데이터 이동
        mv "$PROJECT_DIR" "$ARCHIVE_DIR/"
        
        # 관련 보고서 이동
        if [ -d "reports/research" ]; then
            mv reports/research/${PROJECT_NAME}* "$ARCHIVE_DIR/" 2>/dev/null
        fi
        
        # 압축 옵션
        read -p "압축하시겠습니까? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            tar -czf "${ARCHIVE_DIR}.tar.gz" "$ARCHIVE_DIR"
            rm -rf "$ARCHIVE_DIR"
            echo "✅ 아카이브 완료: ${ARCHIVE_DIR}.tar.gz"
        else
            echo "✅ 아카이브 완료: $ARCHIVE_DIR/"
        fi
        
        # 메타데이터 기록
        cat > "archive/research/${PROJECT_NAME}_metadata.json" << EOF
{
    "project_name": "${PROJECT_NAME}",
    "archived_date": "$(date -Iseconds)",
    "location": "${ARCHIVE_DIR}",
    "status": "archived"
}
EOF
        echo "📝 메타데이터 저장 완료"
        ;;
    
    explore)
        EXPLORATION_TOPIC="$1"
        shift
        DATASET="${1:-compounds.csv}"
        
        if [ -z "$EXPLORATION_TOPIC" ]; then
            echo "❌ 탐구 주제를 지정하세요: /연구 explore \"주제\" [dataset.csv]"
            exit 1
        fi
        
        echo "🔍 투명한 수렴 과정 탐구 시작: $EXPLORATION_TOPIC"
        echo "📊 데이터셋: $DATASET"
        
        # Python 스크립트 실행 환경 확인
        EXPLORE_SCRIPT="scripts/explore_utilities.py"
        if [ ! -f "$EXPLORE_SCRIPT" ]; then
            echo "❌ 탐구 유틸리티를 찾을 수 없습니다: $EXPLORE_SCRIPT"
            echo "   탐구 시스템이 설치되지 않았습니다."
            exit 1
        fi
        
        # Python 실행
        if command -v python3 &> /dev/null; then
            # 임시 데이터 생성 (실제 환경에서는 실제 데이터 로드)
            python3 -c "
import sys
sys.path.insert(0, 'scripts')
from explore_utilities import quick_explore
result = quick_explore('$DATASET', '$EXPLORATION_TOPIC')
print(f'✅ 탐구 완료: {result[\"patterns_found\"]}개 패턴 발견')
print(f'📊 최종 신뢰도: {result[\"confidence\"]:.1%}')
print(f'📁 보고서: {result[\"report_path\"]}')
print(f'🔄 재현: {result[\"reproduction_command\"]}')
"
            
            if [ $? -eq 0 ]; then
                # 연구 진행사항에 기록
                ACTIVE_PROJECT=$(ls -td research_projects/*/ 2>/dev/null | head -1)
                if [ -n "$ACTIVE_PROJECT" ]; then
                    cat >> "${ACTIVE_PROJECT}/timeline.md" << EOF

## $(date '+%Y-%m-%d %H:%M')
🔍 **투명한 탐구 수행**: ${EXPLORATION_TOPIC}
- 데이터셋: ${DATASET}
- 탐구 방법: 투명한 수렴 과정
- 상태: 완료

---
EOF
                    echo "📝 연구 타임라인에 자동 기록됨"
                fi
            else
                echo "❌ 탐구 실행 중 오류가 발생했습니다."
            fi
        else
            echo "❌ Python3이 설치되지 않았습니다."
            exit 1
        fi
        ;;
esac
```

## 🎯 사용 예시

### 짧은 별칭 사용
```bash
# 초단축 버전
/연구 i my_project              # init
/연구 t "오늘 진행사항"           # track
/연구 e baseline "첫 실험"        # experiment
/연구 h "가설 내용"              # hypothesis
/연구 m "마일스톤" "설명"         # milestone
/연구 c "체크포인트 메모"          # checkpoint
/연구 s                         # status
/연구 l                         # list
/연구 f "RGCCA"                 # search
/연구 b                         # backup
/연구 r                         # report
/연구 ex "패턴 분석" data.csv    # explore
```

### 한글 사용
```bash
/연구 시작 프로젝트명 "설명"
/연구 진행 "오늘 한 일"
/연구 실험 exp01 "실험 설명"
/연구 가설 "이렇게 하면 될 것 같다"
/연구 마일스톤 "중요 달성" "상세 설명"
/연구 저장 "현재 상태 백업"
/연구 상태
/연구 목록
/연구 검색 "키워드"
/연구 백업
/연구 보고서
/연구 탐구 "화합물 패턴 분석" compounds.csv
```

### 정식 명령어
```bash
/연구 init project_name "description"
/연구 track "progress note"
/연구 experiment exp_name "description"
/연구 hypothesis "hypothesis content"
/연구 milestone "name" "description"
/연구 checkpoint "memo"
/연구 status
/연구 list
/연구 search "keyword" 
/연구 backup
/연구 report [project_name]
/연구 explore "topic" [dataset.csv]
```

## 📋 별칭 빠른 참조

```
초단축 (1글자):
  i → init      (새 프로젝트)
  t → track     (진행 기록)
  e → experiment (실험)
  h → hypothesis (가설)
  m → milestone  (마일스톤)
  c → checkpoint (체크포인트)
  s → status     (상태)
  l → list       (목록)
  f → search     (검색, find)
  b → backup     (백업)
  r → report     (보고서)
  ex → explore   (탐구, 투명한 수렴)
```

## 🎯 핵심 장점

1. **유연성**: 영어, 한글, 단축어 모두 지원
2. **빠른 입력**: 1글자 별칭으로 빠른 기록
3. **학습 곡선 완화**: 익숙한 언어로 사용
4. **하위 호환**: 기존 명령어 모두 작동

## 💡 Pro Tips

```bash
# 가장 빠른 일일 워크플로우
/연구 t "아침 시작"              # track
/연구 e quick_test "빠른 테스트"  # experiment  
/연구 t "실험 결과: 성공"          # track
/연구 m "Phase 1" "완료"         # milestone
/연구 c "오늘 작업 완료"          # checkpoint

# 한 줄로 여러 작업
/연구 t "진행1" && /연구 t "진행2" && /연구 s

# 보고서 생성 워크플로우
/연구 c "연구 완료 상태 저장"        # checkpoint
/연구 r                           # 자동 보고서 생성
/연구 r my_project                # 특정 프로젝트 보고서

# 투명한 탐구 워크플로우
/연구 i "화합물 연구" "신약 패턴 분석"  # 프로젝트 시작
/연구 ex "약물 유사성 패턴" drugs.csv  # 투명한 탐구 실행
/연구 t "5개 패턴 발견, 신뢰도 92%"    # 결과 기록
/연구 m "탐구 완료" "투명한 수렴 성공"  # 마일스톤
```