...
```bash
# 1. 변수 설정
EXPERIMENT_ID="${1:-current}"
STATUS="${2:-success}"
TIMESTAMP=$(date +%Y-%m-%d_%H%M)

# 2. 실험 경로 확인
if [ "$EXPERIMENT_ID" = "current" ]; then
    EXPERIMENT_PATH=$(readlink research/experiments/current)
    EXPERIMENT_ID=$(basename "$EXPERIMENT_PATH")
else
    EXPERIMENT_PATH="research/experiments/$EXPERIMENT_ID"
fi

# 3. 최종 결과 디렉토리 생성
mkdir -p "${EXPERIMENT_PATH}/FINAL"

# 4. 실험 요약 보고서 생성
cat > "${EXPERIMENT_PATH}/FINAL/summary.md" << 'EOF'
# 실험 완료 보고서: '${EXPERIMENT_ID}'

**완료일**: '$(date +%Y-%m-%d)'
**상태**: '${STATUS}'

## 🎯 목표 달성도
- [실험 목표 대비 달성도를 여기에 작성]
...
# 5. 핵심 발견사항 JSON 생성
cat > "${EXPERIMENT_PATH}/FINAL/key_findings.json" << 'EOF'
{
...
if [ "$STATUS" = "success" ]; then
    ARCHIVE_DIR="research/experiments/completed"
elif [ "$STATUS" = "failed" ]; then
    ARCHIVE_DIR="research/experiments/archived"
else
    ARCHIVE_DIR="research/experiments/partial"
fi

mkdir -p "$ARCHIVE_DIR"
mv "$EXPERIMENT_PATH" "$ARCHIVE_DIR/"
...
echo "- [$(date +%Y-%m-%d)] ${EXPERIMENT_ID}: ${STATUS}" >> docs/CURRENT/research-log.md

# 9. 성공 메시지
echo "✅ 실험 완료: ${EXPERIMENT_ID}"
echo "📁 아카이브 위치: ${ARCHIVE_DIR}/${EXPERIMENT_ID}"
echo "📊 보고서 생성: ${ARCHIVE_DIR}/${EXPERIMENT_ID}/FINAL/summary.md"
```
