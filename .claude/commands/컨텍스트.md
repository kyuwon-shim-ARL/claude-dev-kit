<!--
@meta
id: document_20250905_1110_컨텍스트
type: document
scope: operational
status: archived
created: 2025-09-05
updated: 2025-09-05
tags: commands, 컨텍스트.md, 컨텍스트, .claude
related: 
-->
...
## 6. 🔄 **Compact 가이드 (중요 - 원래 목적)**

**/컨텍스트가 원래 compact 명령을 안내하는 역할이었음을 기억하고 제공합니다:**
...
```bash
/compact
# → 현재 대화를 요약하고 새로운 컨텍스트로 시작
```

**2. 선택적 유지 (Strategic만 보존):**
```bash
/compact "Strategic 영속 컨텍스트와 project_rules.md만 유지. 
이전 구현 세부사항과 Tactical/Operational 컨텍스트는 
아카이브하여 새로운 기획에 집중"
```

**3. 작업 전환 시:**
```bash
/compact "인증 시스템 작업 완료. 
결제 시스템으로 전환하므로 인증 관련 컨텍스트 정리"
```

**4. 토큰 절약 모드:**
```bash
/compact "최소 컨텍스트만 유지. 
project_rules.md와 현재 작업 TODO만 보존"
```
...
### 💡 Compact 활용 팁:
1. **주제 전환 시 필수**: 새로운 기능 개발 시작 전 compact
2. **세션 시작 시**: 이전 대화가 길었다면 compact로 시작
3. **혼란 감지 시**: Claude가 맥락을 놓치면 즉시 compact
4. **정기적 정리**: 매 5-10개 대화마다 한 번씩

### 🚀 자동 Compact 제안:
시스템이 다음 상황을 감지하면 자동으로 compact 제안:
- 토큰 사용률 > 80%
- 주제 전환 감지 (semantic distance > 0.7)
- 에러 반복 발생
- 응답 지연 증가

**💡 핵심: /컨텍스트는 분석하고, /compact는 정리합니다!**

**산출물: 최적화된 컨텍스트 환경 + Claude와의 효율적 협업 기반 구축 + Compact 가이드**